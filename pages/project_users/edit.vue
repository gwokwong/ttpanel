<template>
  <view class="uni-container">
    <uni-forms ref="form" :model="formData" validateTrigger="bind">
      ...
      <uni-forms-item name="owner" label="是否负责人">
        <uni-easyinput placeholder="是否负责人" type="number" v-model="formData.owner"></uni-easyinput>
      </uni-forms-item>
      <uni-forms-item name="top_at" label="置顶时间">
        <uni-datetime-picker return-type="timestamp" v-model="formData.top_at"></uni-datetime-picker>
      </uni-forms-item>
...
<script>
  import { validator } from '../../js_sdk/validator/project_users.js';

  const db = uniCloud.database();
  const dbCmd = db.command;
  const dbCollectionName = 'project_users';
  ...
  export default {
    ...
    methods: {
      ...
      getDetail(id) {
        ...
        db.collection(dbCollectionName).doc(id).field("project_id,userid,owner,top_at,created_at,updated_at,deleted_at").get().then((res) => {
