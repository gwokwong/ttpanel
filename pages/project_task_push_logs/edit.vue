<template>
  <view class="uni-container">
    <uni-forms ref="form" :model="formData" validateTrigger="bind">
      ...
      <uni-forms-item name="type" label="提醒类型：0 任务开始提醒，1 距离到期提醒，2到期超时提醒">
        <uni-easyinput placeholder="提醒类型：0 任务开始提醒，1 距离到期提醒，2到期超时提醒" type="number" v-model="formData.type"></uni-easyinput>
      </uni-forms-item>
...
<script>
  import { validator } from '../../js_sdk/validator/project_task_push_logs.js';

  const db = uniCloud.database();
  const dbCmd = db.command;
  const dbCollectionName = 'project_task_push_logs';
  ...
  export default {
    ...
    methods: {
      ...
      getDetail(id) {
        ...
        db.collection(dbCollectionName).doc(id).field("userid,task_id,type,created_at,updated_at,deleted_at").get().then((res) => {
